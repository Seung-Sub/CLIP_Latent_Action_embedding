...
## 사이클 3 (2026-07-08) — 시그모이드 2×2 (SigLIP2 우위 가설 검증)

동기: 사용자 직관 "SigLIP2가 CLIP보다 나아야" + cowork "sigmoid 일반 우위(SAIL)".
**구현 여정 (검증 규율 자체 사례)**: sigmoid 손실 3형태 시도 — (1) sum: ~40배 과대 →
align 붕괴(a2z 0.5), (2) mean-all: 음성 지배로 과소 → 언어 붕괴(t2a 3.5), (3) **양성/음성
균형 + b₀=−5.5**: 정착. 스케일 버그 2건 모두 자체 발견·수정.

**최종 2×2 (v3 중립어휘, 균형 sigmoid)**:

| | InfoNCE | Sigmoid cw0.3 | Sigmoid cw1.0 |
|---|---|---|---|
| CLIP | t2a 52.7 | 42.4 | 41.8 |
| SigLIP2 | t2a 40.8 | 33.4 | 35.7 |

**판정 (예측 #9 = 반증)**: sigmoid가 두 앵커 모두 InfoNCE 하회, 전 셀 CLIP≥SigLIP2.
"sigmoid 일반 우위"도 "SigLIP2 계열 매칭"도 성립 안 함. **SigLIP2는 이 파이프라인의
어떤 측정(폐루프·오프라인 dec·언어 t2a)에서도 CLIP 미달** → 일반 VL 강도의 비전이
(DINOv2 사례와 동일 주제).

**cowork 확인 요청 (disanalogy)**: SAIL은 image·text 양타워 frozen 정렬. 우리는 g(액션
인코더) 학습 + 텍스트만 frozen인 비대칭. SAIL 시그모이드 이점이 우리 세팅에 전이 안 될
가능성 — 조사 요청.
